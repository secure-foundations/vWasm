...
private (* abstract *) let as_list (#a:eqtype) (#f:cmp a) (s:ordset a f) :(l:list a{sorted f l}) = s
...
private (* abstract *) val insert': #a:eqtype -> #f:cmp a -> x:a -> s:ordset a f
...
let rec union #a #f s1 s2 = match s1 with
  | []     -> s2
...
private (* abstract *) val remove': #a:eqtype -> #f:cmp a -> x:a -> s:ordset a f
             -> Tot (l:(ordset a f){let s = as_list s in let l = as_list l in
	                           ((Nil? s ==> Nil? l) /\
