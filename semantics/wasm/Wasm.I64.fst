...
(**
Wasm-compliant wrappers for I64 arithmetic.

@summary I64 arithmetic.
*)
module Wasm.I64
...
type t = UInt64.t
type bytes = (l: list nat8{List.Tot.length l = 8})
...
let of_bytes bs =
  ...
  let acc, (b :: bs) = acc + (b `op_Multiply` 0x10000), bs in
  let acc, (b :: bs) = acc + (b `op_Multiply` 0x1000000), bs in
  let acc, (b :: bs) = acc + (b `op_Multiply` 0x100000000), bs in
  let acc, (b :: bs) = acc + (b `op_Multiply` 0x10000000000), bs in
  let acc, (b :: bs) = acc + (b `op_Multiply` 0x1000000000000), bs in
  let acc = acc + (b `op_Multiply` 0x100000000000000) in
  of_int_u acc
#pop-options
...
private val to_s: t -> Tot Int64.t
let to_s x = Int64.int_to_t (Int.to_int_t n (v x))

private val fr_s: Int64.t -> Tot t
let fr_s x = uint_to_t (to_uint_t n (Int64.v x))

private let _v = Int64.v
...
let shr_s x y =
  let y' = clamp_mask y in
  if UInt32.v y' = 0 then x else (
    let x' = to_s x in
    let sign = if Int64.lt x' (Int64.int_to_t 0) then of_int_s (-1) else zero in
    let sign_shifted = sign <<^ (UInt32.uint_to_t (n - UInt32.v y')) in
    (x >>^ y') |^ sign_shifted
  )
...
let ne x y = x <> y
let lt_s x y = x <^ y
let lt_u x y = cmp_s x Int64.lt y
let le_s x y = x <=^ y
let le_u x y = cmp_s x Int64.lte y
let gt_s x y = x >^ y
let gt_u x y = cmp_s x Int64.gt y
let ge_s x y = x >=^ y
let ge_u x y = cmp_s x Int64.gte y
