open Prims
type expr = Wasm_Parse_Instr.instr Prims.list


let (check_expr_list_bytesize :
  ...
  fun l ->
    ...
    (FStar_UInt32.lte (FStar_UInt32.uint_to_t Prims.int_zero) x) &&
      (FStar_UInt32.lte x
         (FStar_UInt32.uint_to_t (Prims.parse_int "1048575")))
type expr' =
  (unit, unit, unit, Wasm_Parse_Instr.instr Prims.list, unit, unit)
    LowParse_Spec_VLData.parse_bounded_vldata_strong_t
let (synth_expr : expr' -> expr) = fun x -> x
let (synth_expr_recip : expr -> expr') = fun x -> x
let (expr_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (expr * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    match let res =
    ...
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (expr_serializer32 : expr -> LowParse_SLow_Base.bytes32) =
  fun input ->
    let x = input in
    let pl =
      let uu___ =
...
let (expr_size32 : expr -> FStar_UInt32.t) =
  fun input ->
    let len =
      match let uu___ =
              C_Loops.total_while_gen () ()
