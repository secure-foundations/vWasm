open Prims
type aux_exportdesc_label =
  | Func 
  | Table 
  | Mem 
  | Global 
let (uu___is_Func : aux_exportdesc_label -> Prims.bool) =
  fun projectee -> match projectee with | Func -> true | uu___ -> false
let (uu___is_Table : aux_exportdesc_label -> Prims.bool) =
  fun projectee -> match projectee with | Table -> true | uu___ -> false
let (uu___is_Mem : aux_exportdesc_label -> Prims.bool) =
  fun projectee -> match projectee with | Mem -> true | uu___ -> false
let (uu___is_Global : aux_exportdesc_label -> Prims.bool) =
  fun projectee -> match projectee with | Global -> true | uu___ -> false
let (string_of_aux_exportdesc_label : aux_exportdesc_label -> Prims.string) =
  fun uu___ ->
    match uu___ with
    | Func -> "func"
    | Table -> "table"
    | Mem -> "mem"
    | Global -> "global"
let (aux_exportdesc_label_enum :
  (aux_exportdesc_label, FStar_UInt8.t) LowParse_Spec_Enum.enum) =
...
let (synth_aux_exportdesc_label :
  aux_exportdesc_label -> aux_exportdesc_label) = fun x -> x
let (synth_aux_exportdesc_label_inv :
  aux_exportdesc_label -> aux_exportdesc_label) = fun x -> x


let (parse32_aux_exportdesc_label_key :
  LowParse_SLow_Base.bytes32 ->
    (aux_exportdesc_label * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    ...
    | FStar_Pervasives_Native.Some (k, consumed) ->
        (match k with
         ...
         | uu___ -> FStar_Pervasives_Native.None)
    | uu___ -> FStar_Pervasives_Native.None
let (aux_exportdesc_label_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (aux_exportdesc_label * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    match parse32_aux_exportdesc_label_key input with
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (serialize32_aux_exportdesc_label_key :
  aux_exportdesc_label -> LowParse_SLow_Base.bytes32) =
  fun input ->
...
let (aux_exportdesc_label_serializer32 :
  aux_exportdesc_label -> LowParse_SLow_Base.bytes32) =
  fun input -> let x = input in serialize32_aux_exportdesc_label_key x
let (aux_exportdesc_label_size32 : aux_exportdesc_label -> FStar_UInt32.t) =
