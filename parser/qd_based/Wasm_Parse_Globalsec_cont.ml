open Prims
type globalsec_cont = Wasm_Parse_Aux_vecglobal.aux_vecglobal
let (check_globalsec_cont_bytesize :
  Wasm_Parse_Aux_vecglobal.aux_vecglobal -> Prims.bool) =
  fun x ->
    let l = Wasm_Parse_Aux_vecglobal.aux_vecglobal_size32 x in
    (FStar_UInt32.lte (FStar_UInt32.uint_to_t Prims.int_zero) l) &&
      (FStar_UInt32.lte l
         (FStar_UInt32.uint_to_t (Prims.parse_int "16777215")))
type globalsec_cont' =
  (unit, unit, unit, Wasm_Parse_Aux_vecglobal.aux_vecglobal, unit, unit)
    LowParse_Spec_VLData.parse_bounded_vldata_strong_t
let (synth_globalsec_cont : globalsec_cont' -> globalsec_cont) = fun x -> x
let (synth_globalsec_cont_recip : globalsec_cont -> globalsec_cont') =
  fun x -> x
let (globalsec_cont'_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (globalsec_cont' * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    let res =
            ...
            | FStar_Pervasives_Native.Some (v, l) ->
                ...
                (match if FStar_UInt32.lt (FStar_Bytes.len input') v
                       then FStar_Pervasives_Native.None
                         ...
                         (match Wasm_Parse_Aux_vecglobal.aux_vecglobal_parser32
...
let (globalsec_cont_parser32 :
  ...
  fun input ->
    ...
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (globalsec_cont_serializer32 :
  globalsec_cont -> LowParse_SLow_Base.bytes32) =
  fun input ->
    let x = input in
    let pl = Wasm_Parse_Aux_vecglobal.aux_vecglobal_serializer32 x in
...
let (globalsec_cont_size32 : globalsec_cont -> FStar_UInt32.t) =
  fun input ->
    let len = Wasm_Parse_Aux_vecglobal.aux_vecglobal_size32 input in
