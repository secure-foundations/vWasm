open Prims
type aux_vecglobal = Wasm_Parse_Global.global Prims.list



let (aux_vecglobal_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (aux_vecglobal * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
                                  ...
                                  (match x with
                                   | FStar_Pervasives_Native.Some
                                       ...
                                       then (false, x)
                                         ...
                                         (match Wasm_Parse_Global.global_parser32
                                                  input'1
                                          with
...
let (aux_vecglobal_serializer32 :
  aux_vecglobal -> LowParse_SLow_Base.bytes32) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Global.global_serializer32 a in
...
let (aux_vecglobal_size32 : aux_vecglobal -> FStar_UInt32.t) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Global.global_size32 a in
