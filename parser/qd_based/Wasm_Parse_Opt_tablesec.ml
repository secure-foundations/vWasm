...
type opt_tablesec =
  | X_absent of unit 
  | X_present of Wasm_Parse_Tablesec.tablesec 
let (uu___is_X_absent : opt_tablesec -> Prims.bool) =
  fun projectee ->
    match projectee with | X_absent _0 -> true | uu___ -> false

let (uu___is_X_present : opt_tablesec -> Prims.bool) =
  fun projectee ->
    match projectee with | X_present _0 -> true | uu___ -> false
let (__proj__X_present__item___0 :
  opt_tablesec -> Wasm_Parse_Tablesec.tablesec) =
  fun projectee -> match projectee with | X_present _0 -> _0
let (tag_of_opt_tablesec :
  opt_tablesec -> Wasm_Parse_Optional_tag.optional_tag) =
  fun x ->
    match x with
...
let (optional_tag_as_enum_key :
  Wasm_Parse_Optional_tag.optional_tag ->
    Wasm_Parse_Optional_tag.optional_tag)
  = fun x -> x
let (key_of_opt_tablesec :
  opt_tablesec -> Wasm_Parse_Optional_tag.optional_tag) =
  fun x ->
    match x with
    | X_absent uu___ -> Wasm_Parse_Optional_tag.Absent
    | X_present uu___ -> Wasm_Parse_Optional_tag.Present
type 'x opt_tablesec_case_of_optional_tag = Obj.t
let (to_opt_tablesec_case_of_optional_tag :
  Wasm_Parse_Optional_tag.optional_tag ->
    Wasm_Parse_Optional_tag.optional_tag -> Obj.t -> Obj.t)
  = fun x -> fun x' -> fun y -> y
let (opt_tablesec_refine :
  Wasm_Parse_Optional_tag.optional_tag -> opt_tablesec -> opt_tablesec) =
  fun k -> fun x -> x
let (synth_opt_tablesec_cases :
  Wasm_Parse_Optional_tag.optional_tag -> Obj.t -> opt_tablesec) =
  fun x ->
    fun y ->
      match x with
      | Wasm_Parse_Optional_tag.Absent -> X_absent ()
      | Wasm_Parse_Optional_tag.Present -> X_present (Obj.magic y)
let (from_opt_tablesec_case_of_optional_tag :
  Wasm_Parse_Optional_tag.optional_tag ->
    Wasm_Parse_Optional_tag.optional_tag -> Obj.t -> Obj.t)
  = fun x' -> fun x -> fun y -> y


let (synth_opt_tablesec_cases_recip :
  Wasm_Parse_Optional_tag.optional_tag -> opt_tablesec -> Obj.t) =
  fun k ->
    fun x ->
      match k with
...
let (opt_tablesec_sum : LowParse_Spec_Sum.sum) =
...
let (opt_tablesec_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (opt_tablesec * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    let pi =
    ...
    | FStar_Pervasives_Native.Some (k, consumed_k) ->
        ...
        then
          ...
          (let pcases2 =
             match Wasm_Parse_Tablesec.tablesec_parser32 input_k with
             | FStar_Pervasives_Native.Some (v1, consumed) ->
           ...
           | FStar_Pervasives_Native.Some (x, consumed_x) ->
               FStar_Pervasives_Native.Some
                 (x, (FStar_UInt32.add consumed_k consumed_x)))
let (opt_tablesec_serializer32 : opt_tablesec -> LowParse_SLow_Base.bytes32)
  =
  fun x ->
    let tg =
      match x with
    ...
    let s2 =
      ...
      then FStar_Bytes.empty_bytes
        ...
        Wasm_Parse_Tablesec.tablesec_serializer32
          (match x with | X_present y -> y) in
    let res = FStar_Bytes.append s1 s2 in res
let (opt_tablesec_size32 : opt_tablesec -> FStar_UInt32.t) =
  fun x ->
    let tg =
      match x with
      | X_absent uu___ -> Wasm_Parse_Optional_tag.Absent
    ...
    let s2 =
      ...
      then FStar_UInt32.uint_to_t Prims.int_zero
        ...
        Wasm_Parse_Tablesec.tablesec_size32 (match x with | X_present y -> y) in
