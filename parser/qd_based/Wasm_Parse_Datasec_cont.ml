open Prims
type datasec_cont = Wasm_Parse_Aux_vecdata.aux_vecdata
let (check_datasec_cont_bytesize :
  Wasm_Parse_Aux_vecdata.aux_vecdata -> Prims.bool) =
  fun x ->
    let l = Wasm_Parse_Aux_vecdata.aux_vecdata_size32 x in
    (FStar_UInt32.lte (FStar_UInt32.uint_to_t Prims.int_zero) l) &&
      (FStar_UInt32.lte l
         (FStar_UInt32.uint_to_t (Prims.parse_int "16777215")))
type datasec_cont' =
  (unit, unit, unit, Wasm_Parse_Aux_vecdata.aux_vecdata, unit, unit)
    LowParse_Spec_VLData.parse_bounded_vldata_strong_t
let (synth_datasec_cont : datasec_cont' -> datasec_cont) = fun x -> x
let (synth_datasec_cont_recip : datasec_cont -> datasec_cont') = fun x -> x
let (datasec_cont'_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (datasec_cont' * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    let res =
            ...
            | FStar_Pervasives_Native.Some (v, l) ->
                ...
                (match if FStar_UInt32.lt (FStar_Bytes.len input') v
                       then FStar_Pervasives_Native.None
                         ...
                         (match Wasm_Parse_Aux_vecdata.aux_vecdata_parser32
...
let (datasec_cont_parser32 :
  ...
  fun input ->
    ...
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (datasec_cont_serializer32 : datasec_cont -> LowParse_SLow_Base.bytes32)
  =
  fun input ->
    let x = input in
    let pl = Wasm_Parse_Aux_vecdata.aux_vecdata_serializer32 x in
...
let (datasec_cont_size32 : datasec_cont -> FStar_UInt32.t) =
  fun input ->
    let len = Wasm_Parse_Aux_vecdata.aux_vecdata_size32 input in
