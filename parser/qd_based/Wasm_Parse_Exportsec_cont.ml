open Prims
type exportsec_cont = Wasm_Parse_Aux_vecexport.aux_vecexport
let (check_exportsec_cont_bytesize :
  Wasm_Parse_Aux_vecexport.aux_vecexport -> Prims.bool) =
  fun x ->
    let l = Wasm_Parse_Aux_vecexport.aux_vecexport_size32 x in
    (FStar_UInt32.lte (FStar_UInt32.uint_to_t Prims.int_zero) l) &&
      (FStar_UInt32.lte l
         (FStar_UInt32.uint_to_t (Prims.parse_int "16777215")))
type exportsec_cont' =
  (unit, unit, unit, Wasm_Parse_Aux_vecexport.aux_vecexport, unit, unit)
    LowParse_Spec_VLData.parse_bounded_vldata_strong_t
let (synth_exportsec_cont : exportsec_cont' -> exportsec_cont) = fun x -> x
let (synth_exportsec_cont_recip : exportsec_cont -> exportsec_cont') =
  fun x -> x
let (exportsec_cont'_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (exportsec_cont' * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    let res =
            ...
            | FStar_Pervasives_Native.Some (v, l) ->
                ...
                (match if FStar_UInt32.lt (FStar_Bytes.len input') v
                       then FStar_Pervasives_Native.None
                         ...
                         (match Wasm_Parse_Aux_vecexport.aux_vecexport_parser32
...
let (exportsec_cont_parser32 :
  ...
  fun input ->
    ...
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (exportsec_cont_serializer32 :
  exportsec_cont -> LowParse_SLow_Base.bytes32) =
  fun input ->
    let x = input in
    let pl = Wasm_Parse_Aux_vecexport.aux_vecexport_serializer32 x in
...
let (exportsec_cont_size32 : exportsec_cont -> FStar_UInt32.t) =
  fun input ->
    let len = Wasm_Parse_Aux_vecexport.aux_vecexport_size32 input in
