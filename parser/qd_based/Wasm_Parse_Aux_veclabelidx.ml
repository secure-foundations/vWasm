open Prims
type aux_veclabelidx = Wasm_Parse_Labelidx.labelidx Prims.list



let (aux_veclabelidx_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (aux_veclabelidx * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
                    ...
                    if
                      Prims.op_Negation
                        ((FStar_UInt32.lt v
                           ...
                           ||
                           (FStar_UInt32.lt
                              (FStar_UInt32.uint_to_t (Prims.of_int (255))) v))
                                  ...
                                  (match x with
                                   | FStar_Pervasives_Native.Some
                                       ...
                                       then (false, x)
                                         ...
                                         (match Wasm_Parse_Labelidx.labelidx_parser32
                                                  input'1
                                          with
...
let (aux_veclabelidx_serializer32 :
  aux_veclabelidx -> LowParse_SLow_Base.bytes32) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Labelidx.labelidx_serializer32 a in
...
let (aux_veclabelidx_size32 : aux_veclabelidx -> FStar_UInt32.t) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Labelidx.labelidx_size32 a in
