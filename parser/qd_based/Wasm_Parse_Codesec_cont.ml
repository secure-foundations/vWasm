open Prims
type codesec_cont = Wasm_Parse_Aux_veccode.aux_veccode
let (check_codesec_cont_bytesize :
  Wasm_Parse_Aux_veccode.aux_veccode -> Prims.bool) =
  fun x ->
    let l = Wasm_Parse_Aux_veccode.aux_veccode_size32 x in
    (FStar_UInt32.lte (FStar_UInt32.uint_to_t Prims.int_zero) l) &&
      (FStar_UInt32.lte l
         (FStar_UInt32.uint_to_t (Prims.parse_int "16777215")))
type codesec_cont' =
  (unit, unit, unit, Wasm_Parse_Aux_veccode.aux_veccode, unit, unit)
    LowParse_Spec_VLData.parse_bounded_vldata_strong_t
let (synth_codesec_cont : codesec_cont' -> codesec_cont) = fun x -> x
let (synth_codesec_cont_recip : codesec_cont -> codesec_cont') = fun x -> x
let (codesec_cont'_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (codesec_cont' * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
    let res =
            ...
            | FStar_Pervasives_Native.Some (v, l) ->
                ...
                (match if FStar_UInt32.lt (FStar_Bytes.len input') v
                       then FStar_Pervasives_Native.None
                         ...
                         (match Wasm_Parse_Aux_veccode.aux_veccode_parser32
...
let (codesec_cont_parser32 :
  ...
  fun input ->
    ...
    | FStar_Pervasives_Native.Some (v1, consumed) ->
        FStar_Pervasives_Native.Some (v1, consumed)
    | uu___ -> FStar_Pervasives_Native.None
let (codesec_cont_serializer32 : codesec_cont -> LowParse_SLow_Base.bytes32)
  =
  fun input ->
    let x = input in
    let pl = Wasm_Parse_Aux_veccode.aux_veccode_serializer32 x in
...
let (codesec_cont_size32 : codesec_cont -> FStar_UInt32.t) =
  fun input ->
    let len = Wasm_Parse_Aux_veccode.aux_veccode_size32 input in
