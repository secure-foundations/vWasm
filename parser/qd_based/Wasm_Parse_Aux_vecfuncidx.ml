open Prims
type aux_vecfuncidx = Wasm_Parse_Funcidx.funcidx Prims.list



let (aux_vecfuncidx_parser32 :
  LowParse_SLow_Base.bytes32 ->
    (aux_vecfuncidx * FStar_UInt32.t) FStar_Pervasives_Native.option)
  =
  fun input ->
                    ...
                    if
                      Prims.op_Negation
                        ((FStar_UInt32.lt v
                           ...
                           ||
                           (FStar_UInt32.lt
                              (FStar_UInt32.uint_to_t (Prims.of_int (1023)))
                              v))
                                  ...
                                  (match x with
                                   | FStar_Pervasives_Native.Some
                                       ...
                                       then (false, x)
                                         ...
                                         (match Wasm_Parse_Funcidx.funcidx_parser32
                                                  input'1
                                          with
...
let (aux_vecfuncidx_serializer32 :
  aux_vecfuncidx -> LowParse_SLow_Base.bytes32) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Funcidx.funcidx_serializer32 a in
...
let (aux_vecfuncidx_size32 : aux_vecfuncidx -> FStar_UInt32.t) =
  fun x ->
    let ln =
      match let uu___ =
              C_Loops.total_while_gen () ()
            ...
            (fun uu___1 ->
               ...
               | (uu___2, x2) ->
                   (match x2 with
                    | ([], res) -> (false, x2)
                    | (a::q, res) ->
                        let sa = Wasm_Parse_Funcidx.funcidx_size32 a in
