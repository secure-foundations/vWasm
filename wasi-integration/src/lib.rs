...
static mut PTR_MEMORY: Option<*mut u8> = None;
/// Pointer to number of Wasm 64K pages currently active in Wasm memory.
static mut PTR_MEM_PAGES: Option<*mut u64> = None;
...
/// Arguments:
///
/// + `ptr_memory`: Pointer to the main Wasm linear memory.
/// + `ptr_mem_pages`: Pointer to a `u64` keeping track of number of
///    active pages in linear memory.
#[no_mangle]
pub extern "C" fn wasi_initialize_context(ptr_memory: *mut u8, ptr_mem_pages: *mut u64) {
    let ctx =
    ...
    let old = unsafe { PTR_MEMORY.replace(ptr_memory) };
    assert_eq!(old, None);
...
fn get_memory() -> GuestMemWrapper<'static> {
    GuestMemWrapper::new(unsafe {
        ...
        let mem_addr: *mut u8 = PTR_MEMORY.expect(IMPOSSIBLE);
        let mem_pages: u64 = *PTR_MEM_PAGES.expect(IMPOSSIBLE);
